...
inc, dec, neg, not（一元）、add, sub, imul, xor, or, and（二元）中，源和目的都可以是内存地址，**但不能同时都是内存地址**
...
数组行优先，也就是说在内存中按照A[0][0],A[0][1],...,A[1][0],...的方式存储

理解 `int *A[3][5]`、`int (*A[3])[5]`和`int (*A)[3][5]`的区别：
* `int *A[3][5]`表示一个3行5列的数组，数组元素都是int*指针
* `int (*A[3])[5]` 表示一个包括3个元素的数组，每个元素都是指向‘长为5的int数组’的指针
* `int (*A)[3][5]` 表示一个指向‘3行5列的int数组’的指针

#### b. 变长数组与变长栈帧

malloc创建的数组在运行时堆上，而以变量指定长度的数组在用户栈上

#### c. 结构与联合

gcc对结构的传参可能会利用1~2个寄存器（比方说`typedef struct{char a[3];int m;int n;} struct_1;`，那么a和m在rdi里，n在esi里。（*卷子再坑也应该不会考这么细*）

x86-64中，结构和联合中对齐的规则：k字节的基本对象的地址必须是k的倍数。因而，一个结构的sizeof并不是它所有成员的sizeof的和

*IA32 Linux和Windows下默认的对齐原则与上面不同*
...
PF是啥 jp是啥 CSAPP3eZN p.213~p.214
